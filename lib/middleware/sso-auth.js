...
export async function authenticateSSORequest(request, requiredRoles = [], options = {}) {
  try {
    ...
    const ssoAccessToken = cookieStore.get(COOKIE_ACCESS)?.value || cookieStore.get('sso_access_token')?.value;
    const ssoUserCookie = cookieStore.get(COOKIE_USER)?.value || cookieStore.get('sso_user')?.value;
    ...
    try {
      ...
      user.userId = user.userId || cookieUser.id || cookieUser.userId || user.id;
      user.role = user.role || cookieUser.role || (user.roles && user.roles[0]) || 'admin';
      if (cookieUser.role && (!user.roles || !user.roles.includes(cookieUser.role))) {
        user.roles = user.roles || [];
        user.roles.push(cookieUser.role);
      }
    } catch (e) {
