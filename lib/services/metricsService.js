...
export async function getDashboardMetrics(rlsClient) {
  try {
    ...
    logger.debug('No metrics snapshot found, calculating dynamically');
    ...
    if (universitiesError) {
      logger.error('Error fetching universities', { error: getErrorMessage(universitiesError) });
    }
    ...
    logger.error('Error fetching metrics', { error: getErrorMessage(error) });
    return {
