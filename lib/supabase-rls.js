...
export async function getSession() {
  ...
  if (error || !user) {
    // Fallback: SSO JWT auth (sp-dash uses SSO login, not Supabase Auth)
  ...
  if (userError || !userData) {
    ...
    // Return basic auth user data if custom user data fetch fails or user not found in users table
    return {
      user: {
        id: user.id,
        email: user.email,
  ...
  // Fetch admin role — takes precedence over users.role for sp-dash admin users
