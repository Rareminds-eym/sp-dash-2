...
export async function getUserPermissions(userId) {
  try {
    // Fetch user with role - first check admin_users table
    ...
    // If not found in admin_users, user doesn't have permissions
    if (!adminUser) {
      return []
    }

    const user = { role: adminUser.admin_role }
...
/**
 * Original getUserDetails kept for backward compatibility
 */
export async function getUserDetailsOld(userId) {
  try {
...
export async function canAccessEntity(userId, entityType, entityId) {
  ...
  // For now, all admin users can access all entities
  // This can be refined later with entity-specific access control
