...
export class SSOServiceClient {
  ...
  /**
   * Admin-provisioned update of an existing active/pending Hybrid
   * subscription's commercial terms. Used when the admin edits negotiated
   * terms (price, seats, billing cycle, features, notes) for an org that
   * is already on the Hybrid plan. Same admin-role requirement as
   * createHybridSubscription.
   * @param {Object} params - { organization_id, plan_amount, seat_count, billing_cycle?, features?, notes?, admin_user_id }
   * @returns {Promise<Object>} the updated subscription fields
   */
  async updateHybridSubscription(params) {
    if (!this.binding) {
      throw new Error('SSO service binding not available');
    }
    return await this.binding.updateHybridSubscription(params);
  }
  ...
  /**
   * List the canonical admin-dashboard feature key catalog — the source of
   * truth for what can be granted to a Hybrid org (see feature_keys table).
   * Used to populate the feature checkboxes in the New Hybrid Organization
   * and Upgrade-to-Hybrid dialogs, instead of a free-text field.
   * @returns {Promise<Object>} { featureKeys: [{ id, product_id, key, role, nav_group, nav_label, nav_path, display_order }] }
   */
  async listFeatureKeys() {
    if (!this.binding) {
      throw new Error('SSO service binding not available');
    }
    return await this.binding.listFeatureKeys();
  }
