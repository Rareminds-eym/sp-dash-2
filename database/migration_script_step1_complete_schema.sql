...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='students' AND column_name='current_cgpa') THEN
        ALTER TABLE students ADD COLUMN current_cgpa DECIMAL(4,2);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='students' AND column_name='user_id') THEN
        ALTER TABLE students ADD COLUMN user_id UUID REFERENCES users(id) ON DELETE CASCADE;
    END IF;
END $$;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='recruiters' AND column_name='user_id') THEN
        ALTER TABLE recruiters ADD COLUMN user_id UUID REFERENCES users(id) ON DELETE CASCADE;
    END IF;
