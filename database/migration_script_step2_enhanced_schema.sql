...
CREATE INDEX IF NOT EXISTS idx_student_enrollments_school_class ON student_enrollments(school_class_id);
CREATE INDEX IF NOT EXISTS idx_student_enrollments_college_course ON student_enrollments(college_course_id);
CREATE INDEX IF NOT EXISTS idx_student_enrollments_university_course ON student_enrollments(university_course_id);
CREATE INDEX IF NOT EXISTS idx_student_enrollments_dates ON student_enrollments(enrollment_date, expected_graduation_date);
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(sc.name, cc.name, uc.name) as class_name,
        se.enrollment_date,
        se.expected_graduation_date
    FROM student_enrollments se
    LEFT JOIN schools s ON se.school_id = s.id
    LEFT JOIN colleges c ON se.college_id = c.id
    LEFT JOIN universities u ON se.university_id = u.id
    LEFT JOIN school_classes sc ON se.school_class_id = sc.id
    LEFT JOIN college_courses cc ON se.college_course_id = cc.id
    LEFT JOIN university_courses uc ON se.university_course_id = uc.id
...
BEGIN
    ...
    IF NEW.school_id IS NOT NULL THEN
        UPDATE schools SET total_students = (
            SELECT COUNT(DISTINCT student_id) FROM student_enrollments 
            WHERE school_id = NEW.school_id AND enrollment_status = 'active'
        )
    ...
    IF NEW.college_id IS NOT NULL THEN
        UPDATE colleges SET total_students = (
            SELECT COUNT(DISTINCT student_id) FROM student_enrollments 
            WHERE college_id = NEW.college_id AND enrollment_status = 'active'
        )
    ...
    IF NEW.university_id IS NOT NULL THEN
        UPDATE universities SET total_students = (
            SELECT COUNT(DISTINCT student_id) FROM student_enrollments 
            WHERE university_id = NEW.university_id AND enrollment_status = 'active'
        )
...
SELECT 
    ...
    CASE 
        ...
        WHEN se.university_id IS NOT NULL THEN 'university'
    END as entity_type,
    COALESCE(sch.id, col.id, uni.id) as entity_id,
...
LEFT JOIN schools sch ON se.school_id = sch.id
LEFT JOIN colleges col ON se.college_id = col.id
...
CREATE INDEX IF NOT EXISTS idx_enrollments_student_status_date ON student_enrollments(student_id, enrollment_status, enrollment_date DESC);
CREATE INDEX IF NOT EXISTS idx_enrollments_active ON student_enrollments(student_id) WHERE enrollment_status = 'active';
