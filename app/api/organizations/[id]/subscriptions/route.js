...
/**
 * PATCH /api/organizations/[id]/subscriptions
 *
 * Admin-provisioned update of an existing active/pending Hybrid
 * subscription's commercial terms (price, seats, billing cycle, features,
 * notes). Used when the admin edits negotiated terms for an org that is
 * already on the Hybrid plan.
 *
 * Body: { plan_amount: number, seat_count: number, billing_cycle?: string,
 *         features?: unknown[], notes?: string }
 */
export async function PATCH(request, { params }) {
  try {
    ...
    let result;
    try {
      result = await ssoClient.updateHybridSubscription({
        organization_id: organizationId,
        plan_amount,
        seat_count,
        billing_cycle: billing_cycle || undefined,
      ...
      logger.error('updateHybridSubscription RPC error', {
        error: rpcError.message,
        organizationId,
        adminUserId: user.id,
      });
      const message = rpcError.message || 'Failed to update Hybrid subscription';
      const status = message.includes('not found') ? 404 : 500;
      return NextResponse.json({ error: message }, { status });
    }

    logger.info('Hybrid subscription terms updated', {
      organizationId,
      adminUserId: user.id,
    ...
    logger.error('Error updating Hybrid subscription', {
      error: err instanceof Error ? err.message : String(err),
      stack: err instanceof Error ? err.stack : undefined,
    });
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
